import ImageList from '@mui/material/ImageList';
import ImageListItem from '@mui/material/ImageListItem';
import ImageListItemBar from '@mui/material/ImageListItemBar';
import Image from 'next/image'
import styles from '../styles/Popular.module.css'
import Link from 'next/link';

export default function MainList({ articles }) {

    const quilted = [
        {
            rows: 2,
            cols: 2
        },
        {
            rows: 1,
            cols: 1
        },
        {
            rows: 1,
            cols: 1
        },
        {
            cols: 1
        },
        {
            cols: 1
        },
        {
            rows: 1,
            cols: 1
        },
        {
            rows: 1,
            cols: 1
        },
        {
            rows: 2,
            cols: 2
        },
        {
            rows: 2,
            cols: 2
        },
        {
            rows: 1,
            cols: 1
        },
        {
            rows: 1,
            cols: 1
        },
        {
            cols: 2,
        },
        {
            rows: 2,
            cols: 2
        },
        {
            rows: 1,
            cols: 1
        },
        {
            rows: 1,
            cols: 1
        },
        {
            cols: 2,
            rows: 2
        },
        {
            cols: 1,
            rows: 1
        },
        {
            cols: 1,
            rows: 1
        }

    ]

    return (

        <ImageList className={styles['article-list']}
            variant="quilted"
            gap={40}
            cols={4}
            rowHeight={150}
        >
            {articles && articles.map((article, idx) => (

                <ImageListItem className={styles['article-card']}
                    sx={{ height: 'auto !important' }}
                    key={article.id}
                    cols={quilted[idx]?.cols}
                    rows={quilted[idx]?.rows}
                >
                    <Link href={{
                        pathname: '/article/[id]',
                        query: { id: article.uri.replace(/'/, '"') }
                    }}
                       /*  as={`/article/${article.id}`} */>
                        <a>
                            
                            <Image className={styles['article-img']}
                                src={`${article.media[0] ? article.media[0]['media-metadata'][2]?.url : 'data:image/jpeg;base64,'}`}
                                alt={article.title}
                                loading="lazy"
                                layout='fill'
                            />

                            <ImageListItemBar className={styles['article-bar']}
                                title={article.title}
                                subtitle={article.published_date}
                            />
                        </a>
                    </Link>
                </ImageListItem>
            ))}
        </ImageList>
    )
}